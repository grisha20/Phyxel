...
Статус документа: архитектурное решение для реализации после baseline `242e8ff`. Исполняемый код и материалы этим документом не изменяются.

Первая проверочная цепочка будущей реализации:
...
Рекомендуется маленький GPU summary, а не система счётчиков:

- один `uint` с битами `Occurred`, `TargetCellular`, `TouchesSolid`, `TargetLiquid`, `TargetGas`, `TargetMovableSolid`;
...
- coordinator использует кольцо минимум из двух query/staging slots;
...
При ненулевом summary coordinator должен установить `presentationDirty`, `cellMaterialsDirty` и `waterPressureRoutesDirty`, сбросить cellular rest-finalization и разбудить нужные schedules. При затронутом solid нужно установить `topologyDirty`; при target `movable-solid` component labeling обязателен до solid-body pass.
...
Циклы в графе нужны и разрешены, если hysteresis делает невозможным немедленное повторение при неизменной температуре. Для каждого цикла validator пересекает открытые температурные интервалы его рёбер:

- `above(t)` добавляет ограничение `T > t`;
- `below(t)` добавляет ограничение `T < t`;
- также действует общий диапазон `-273.15…5000`.

Если пересечение цикла непусто, одна и та же температура могла бы бесконечно обходить цикл по одному ребру за кадр; такой набор отклоняется. Циклы `water below 0 → ice` / `ice above 2 → water` и `water above 100 → steam` / `steam below 95 → water` допустимы, потому что соответствующие пересечения пусты.
...
`core:water` получает `below 0 → core:ice` и `above 100 → core:steam` без других изменений.
...
2. **Run universal phase transition GPU pass** — `PhaseTransitions.hlsl`, GPU resources, in-place dispatch, summary/query ring, coordinator order, Pause semantics, CPU normalizer/scheduler regression и timestamps. Без core ice/steam.
3. **Add phase transition GPU acceptance** — external acceptance-only materials и сценарии generic kind-пар, hysteresis, one-transition, field normalization, runtime reorder и missing target. Без пользовательского core content.
4. **Add water ice steam phase chain** — только core JSON для ice/steam и transitions в water, UI order и content acceptance. Общий shader не меняется.
5. **Complete phase transition compatibility coverage** — v5 round-trip, старые save cases, полный regression matrix и обновление общей документации.
...
## 14. Открытые вопросы для Степана

Перед кодом желательно явно подтвердить четыре продуктовых решения:

1. Приемлем ли неподвижный лёд в первой версии, пока movable-solid topology не доказана acceptance-тестами?
2. Приемлема ли модель A с непрерывной температурой, но без сохранения полной sensible/latent energy на границе фазы?
3. Подтверждается ли cadence: один phase pass после всей thermal-пачки кадра, а не после каждого diffusion tick?
4. Подтверждается ли запрет `into: "core:empty"` в phase v1 и текущая save-политика missing palette material → empty с warning?

Все остальные решения документа можно реализовать без material-specific веток и без изменения `GridCell`/v5.
