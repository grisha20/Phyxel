...
Статус документа: schema/registry/layout-слой реализован коммитом `Add phase transition material schema`; GPU pass, phase resources и пользовательские phase-материалы остаются следующими этапами.
...
Сразу после будущего phase dispatch coordinator должен консервативно установить `presentationDirty=true`, не ожидая асинхронный GPU summary: смена материала уже могла изменить композицию текущего кадра. При ненулевом summary coordinator также должен установить `cellMaterialsDirty` и `waterPressureRoutesDirty`, сбросить cellular rest-finalization и разбудить нужные schedules. При затронутом solid нужно установить `topologyDirty`; при target `movable-solid` component labeling обязателен до solid-body pass.
