...
// core:steam
"transitions": {
  "below": { "temperature": 98.0, "into": "core:water" }
}
```

Таким образом, вода замерзает ниже `0 °C`, лёд тает выше `2 °C`, вода кипит выше `100 °C`, а пар конденсируется ниже `98 °C`. В промежутках `0…2` и `98…100 °C` материал сохраняет текущую фазу.
...
Текущий gas solver позволяет steam локально подниматься и распространяться в соседние empty-клетки. Клетка переносится целиком без split/merge, поэтому `MaterialIndex`, `Mass`, `Temperature` и `Lifetime` пакета сохраняются. Разные газовые материалы не смешиваются, а конденсация выполняется тем же общим phase pass.

Строгая массовая acceptance теперь требует точного сохранения количества целых газовых пакетов; отдельного `GasMinimumMass` в solver нет. Сам phase pass массу не меняет.

Bundled core-набор теперь содержит 10 определений. Фактические runtime-индексы текущего отсортированного набора — `empty=0`, `gas=3`, `ice=4`, `steam=7`, `water=9`; эти числа являются диагностическим снимком, а не контрактом. Все ссылки переходов разрешаются из string ID при каждом запуске. В UI материалы идут подряд как Water 20, Ice 21 и Steam 22; обычная кисть создаёт их при 20, −5 и 122 °C соответственно.
...
- более плотные smoke и CO₂ используют общий массовый solver;
