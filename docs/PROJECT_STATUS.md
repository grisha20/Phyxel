...
|---|---|---:|
| Фаза 0 — стабилизация ядра | В работе | около 55–60% |
| Фаза 1 — материалы наружу | Завершена | 100% |
| Фаза 2 — универсальные системы | В работе | около 25–30% |
| Фаза 3 — реакции из JSON | Не начата | 0% |
| Фаза 4 — локальные пакеты | Не начата | 0% |
| Фаза 5 — официальный хаб | Не начата | 0% |

От исходного общего плана выполнено примерно **35%**. Если считать только параллельную архитектуру простых материалов без реакций и фазовых переходов, она готова примерно на **80%**. Эти значения нужны для планирования и должны пересматриваться после завершения очередной универсальной системы.
...
- Все core-материалы находятся в `Materials/core`: empty, sand, water, metal, stone, gas, fixture и UI-инструмент eraser.
...
1. Завершить и зафиксировать текущую документацию.
2. Отдельно проанализировать архитектуру фазовых переходов до написания кода.
3. Добавить core-материалы ice и steam.
4. Реализовать `water ↔ ice ↔ steam` через свойства материалов, без жёстких ID в shader-коде.
5. Добавить CPU/GPU acceptance фазовых переходов и совместимости сохранений.
6. Добавить CO₂ как проверку универсальности газа.
7. Спроектировать и реализовать универсальное горение.
8. Добавить wood/smoke/coal через данные и acceptance.
9. Спроектировать JSON-реакции и общую GPU-таблицу правил.
10. Реализовать взрывы как отдельную универсальную механику.
11. Реализовать коррозию и растворение.
12. Ввести локальные Packs.
13. Только после стабилизации Packs начинать hub.

Фазовые переходы требуют анализа перед кодом: сначала зафиксировать JSON-схему, GPU-проходы и правила для движущихся клеток. Аналогично реакции не следует смешивать с первой реализацией фаз.
...
- Нет огня, источников постоянного нагрева и фазовых переходов.
