...
- cellular gas переносит `Mass` целым локальным пакетом без split/merge, а liquid/granular используют её как количество/заполнение. Поэтому эти source kinds нельзя включать в v1 без отдельного acceptance.
...
- flame-клетка не проходит через обычное пакетное gas-движение: она не смешивается со smoke/CO₂ и поднимается преимущественно вверх с небольшим `DeltaTime`-ограниченным диагональным дрейфом;
